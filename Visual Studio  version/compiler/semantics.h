...
void check(TreeNode* p) {
	if (p == NULL || p->type == epsilon) return;
	switch (p->type) {
		case programType: check_program(p); break;
		case stmtblockType: check_stmtblock(p); break;
		case extdefsType: check_extdefs(p); break;
		case extdefType: check_extdef(p); break;
		case sextvarsType: check_sextvars(p); break;
		case extvarsType: check_extvars(p); break;
		case stspecType: check_stspec(p); break;
		case funcType: check_func(p); break;
		case typeToken: check_type(p); break;
		case stmtsType: check_stmts(p); break;
		case stmtType: check_stmt(p); break;
		case defsType: check_defs(p); break;
		case sdefsType: check_sdefs(p); break;
		case decsType: check_decs(p); break;
		case varType: check_var(p); break;
		case initType: check_init(p); break;
		case expType: check_exp(p,NULL); break;
		case expsType: check_exps(p); break;
		case arrsType: check_arrs(p); break;
		case biexpType: break;
		case intToken: break;
		case sdecsType: check_sdecs(p,0); break;
...
void check_sextvars_1(char *s, TreeNode *p) {
	...
	while (1) {
		if (env[tmp_level][tmp_depth].struct_id_table.find(s)!=env[tmp_level][tmp_depth].struct_id_table.end()) { 
			...
			int parent = env[tmp_level][tmp_depth].parent_index;
			if (parent==-1||tmp_level==0) {report_err("No such struct named~",s,p->lineNum);}
			else {
				tmp_level--;
				tmp_depth = parent;
			}
	...
	if (check_id(p->children[0]->data,p->children[0]->lineNum)&&func_table.find(p->children[0]->data)!=func_table.end()) {
		report_err("redefinition of",p->children[0]->data,p->children[0]->lineNum);
	}
	else {
	...
	if (p->size>=1) {
		...
		if (env[level+1][cnt[level+1]+1].table.find(p->children[0]->data)==env[level+1][cnt[level+1]+1].table.end()) {
		 // to see whether multiple declarations of a variable
			...
			report_err("redefinition of", p->children[1]->data,p->children[1]->lineNum);
		}
	}
...
void check_func(TreeNode *p) { 
	if (p == NULL) return;
	++func_cnt;
	if (check_id(p->children[0]->data,p->children[0]->lineNum)) { // no conflict with the name of int variables and struct variables
		tmp_num_of_var = 0;
		...
		if (func_table.find(p->children[0]->data)==func_table.end()) { // new function name
			...
			else report_err("multiple definitions of", p->children[0]->data,p->children[0]->lineNum);
		}
		
	}
	else {
		report_err("conflicting names of", p->children[0]->data,p->children[0]->lineNum);
	}
}
...
void check_var(TreeNode *p) {//OK
	if (p == NULL) return;
	if (p->size==1) {
		if (check_id(p->children[0]->data,p->children[0]->lineNum)) {  //new variable name
		...
		else {report_err("redefinition of", p->children[0]->data,p->children[0]->lineNum);}
	}
	else {
		int intnum = StringToInt(p->children[1]->data);
		if (intnum < 0) report_err("index can't be less than zero",NULL,p->lineNum);
...
void check_exps(TreeNode *p) {
	...
	if (!strcmp(p->data,"exps f()")) {
		if (func_table.find(p->children[0]->data)==func_table.end()) { //don't find the function
			report_err(p->children[0]->data,"not declared",p->children[0]->lineNum);
		}
		else {
		...
		if (vector_find(func_table[p->children[0]->data],func_cnt_table[p->children[0]->data])) {
				...
				report_err("number of arguments doesn't match: function", p->children[0]->data,p->children[0]->lineNum);
			}
		...
		if (!find_id(p->children[0])) {
			report_err("No variable named",p->children[0]->data,p->children[0]->lineNum);
		}
		if (find_struct_id(p->children[0]->data,NULL)) {
				report_err(p->children[0]->data,"is a struct",p->children[0]->lineNum);
		}
		if (p->children[1]==NULL || p->children[1]->size==0) {
			if (get_width_semantics(p->children[0]->data)!=1) {
				report_err(p->children[0]->data,"is not an integer",p->children[0]->lineNum);
			}
		}
		else if (p->children[1]->size!=0) {  // it should be an array
			if (get_width_semantics(p->children[0]->data)==1) {
				report_err(p->children[0]->data,"is not array",p->children[0]->lineNum);
			}
		}
	}
		...
		if (sjtu==0) {
			report_err("No such struct named", p->children[0]->data,p->children[0]->lineNum);
		}
		else if (sjtu==-1){
			report_err("struct has no varibale named", p->children[2]->data,p->children[2]->lineNum);
		}
	}
...
void check_left_value_exps(TreeNode *p) {
	...
	else {report_err("lvalue required as left operand assignment",NULL,p->lineNum);}
}
...
void check_stmt(TreeNode *p) {
	...
	if (!strcmp("stmt",p->data)||!strcmp(p->data,"stmt: exp;")) {
		...
		if (!in_func) report_err("return not in a function",NULL,p->lineNum);
		check_exps(p->children[1]);
	}
		...
		if (!in_for) {
			report_err(p->children[0]->data,"not in a loop",p->children[0]->lineNum);
		}
	}
}
